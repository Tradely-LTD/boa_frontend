...
const ownerNav = [
  { label: 'Dashboard',  icon: 'dashboard',     to: '/shop/dashboard' },
  { label: 'Inventory',  icon: 'inventory_2',   to: '/shop/inventory' },
  { label: 'Sales',      icon: 'point_of_sale', to: '/shop/sales' },
  { label: 'Expenses',   icon: 'receipt',       to: '/shop/expenses' },
  { label: 'My Staff',   icon: 'group',         to: '/shop/staff' },
];
...
export default function ShopLayout() {
  ...
  const navItems = user?.role === 'shop_owner' ? ownerNav : repNav;
  const portalLabel = user?.role === 'shop_owner' ? 'Shop Portal' : 'Sales Portal';
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 overflow-x-hidden">
      ...
      <aside className={`fixed left-0 top-0 h-screen w-64 bg-boa-green-dark flex flex-col z-30
        ...
        <div className="px-5 py-4 border-b border-emerald-900 flex items-center justify-between">
          <div>
            ...
            <p className="text-emerald-400 text-xs">{portalLabel}</p>
          </div>
        ...
        <div className="px-4 py-4 border-t border-emerald-900">
          <div className="flex items-center gap-3 mb-3 px-1">
            <div className="w-8 h-8 rounded-full bg-emerald-800 flex items-center justify-center shrink-0">
              <span className="material-symbols-outlined text-base text-emerald-300">storefront</span>
            </div>
      ...
      <div className="flex-1 min-w-0 md:ml-64 flex flex-col min-h-screen">
        <header className="sticky top-0 z-20 bg-white border-b border-slate-100 px-4 md:px-8 py-3 flex items-center gap-3">
          <button onClick={() => setSidebarOpen(true)}
            className="md:hidden p-1.5 rounded-lg text-slate-500 hover:bg-slate-100 transition shrink-0">
            <span className="material-symbols-outlined">menu</span>
          </button>
          <p className="text-sm font-semibold text-slate-700 truncate">{portalLabel}</p>
        </header>
